...
ui <- bslib::page_sidebar(
  ...
  sidebar = bslib::sidebar(
    ...
    conditionalPanel(
      ...
      conditionalPanel(
        ...
        #             # improve label
        #             label = "Select the number of columns to display:",
        #             min = 1, max = 20, value = 10)
        selectInput("bins", selectize = FALSE,
                    label = "Select the number of columns to display:", 
  ...
  mainPanel(
    tabsetPanel(id = "my_tabs", 
                ...
                tabPanel(
                  title = "Barplot", value = "tab_bar", 
                  h2("Drawing an interactive barplot of iris measurements"),
                  p('Select filters in the sidebar to control iris species, 
                     measure, and measure limits to display. If the sidebar is 
                     not visible, open it by clicking the chevron (">") or 
                ...
                tabPanel(
                  title = "Histogram", value = "tab_hist", 
                  h2("Drawing an interactive histogram of iris measurements"),
                  p('Select filters in the sidebar to control the iris species, 
                     measure, and measure limits to display. If the sidebar is 
                     not visible, open it by clicking the chevron (">") or 
                     "toggle sidebar" label.'),
...
server <- function(input, output, session) {
  ...
  output$bar_desc <- shiny::renderText({
    d <- filtered_data()
    desc <- paste("This bar chart shows", input$species, "irises filtered on", 
                  input$measure, "ranging from", min(d$measure), "to", 
                  max(d$measure), "centimeters.")
